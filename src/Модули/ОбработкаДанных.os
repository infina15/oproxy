...
//строка "POST" в виде двоичных данных
Перем МаркерНачалаЗапроса;
Перем МаркерПоискаЗаголовков;
...
Процедура ПриСозданииОбъекта()
	...
	МаркерНачалаЗапроса = "POST";
	МаркерПоискаЗаголовков = ПолучитьСтрокуИзДвоичныхДанных(ПолучитьДвоичныеДанныеИзHEXСтроки("0D0A0D0A0D0A"));
КонецПроцедуры

Функция ПолучитьРазмерЗапроса(ВходныеДанные) Экспорт
	Заголовки = ПолучитьЗаголовки(ВходныеДанные);
	Результат = 0;
	Если Заголовки <> Неопределено Тогда
		Результат = Заголовки["Content-Length"];
	КонецЕсли;
	Возврат Результат;
КонецФункции

Функция ПолучитьЗаголовки(ВходныеДанные) Экспорт
	Чтение = Новый ЧтениеТекста(ВходныеДанные.ОткрытьПотокДляЧтения());
	Результат = Чтение.ПрочитатьСтроку();
	Если НЕ СтрНачинаетсяС(Результат, "POST") Тогда
		Чтение.Закрыть();
	...
	Результат = Чтение.ПрочитатьСтроку();
	РазделенныйЗаголовок = СтрРазделить(Результат, ":");
	Заголовки.Вставить("Content-Length", Число(РазделенныйЗаголовок[1]));
	Результат = Чтение.ПрочитатьСтроку();
	РазделенныйЗаголовок = СтрРазделить(Результат, ":");
	Заголовки.Вставить("Content-Type", СокрЛП(РазделенныйЗаголовок[1]));
	Результат = Чтение.ПрочитатьСтроку();
	РазделенныйЗаголовок = СтрРазделить(Результат, ":");
	Заголовки.Вставить("Accept", СокрЛП(РазделенныйЗаголовок[1]));
...
Функция ЭтоНачалоЗапросаДанных(ВходныеДанные) Экспорт
	//потому что все, что кроме POST - это пинг
	Если ВходныеДанные.Размер() < 14 Тогда
		Возврат Ложь;
	КонецЕсли;
	Поток = ВходныеДанные.ОткрытьПотокДляЧтения();
	Буфер = Новый БуферДвоичныхДанных(14);
	Поток.Прочитать(Буфер, 0, 14);
	Поток.Закрыть();
	Возврат ПолучитьДвоичныеДанныеИзСтроки("POST  HTTP/1.1") = ПолучитьДвоичныеДанныеИзБуфераДвоичныхДанных(Буфер);
КонецФункции
...
Функция ЕстьКонецСообщения_Стар(ВходныеДанные)
	//наличие конца сообщения может быть:
	//1. концом сообщения (СигнатураПоискаКонца)
	//2. концом сообщения (СигнатураПоискаКонца) + несколько пингов (СигнатураПоискаПинга)
	//поэтому надо искать с конца: если это последовательность пингов + сигнатура конца, то истина
	//иначе ложь
	Поток = ВходныеДанные.ОткрытьПотокДляЧтения();
	Размер = Поток.Размер();
...
Функция ЕстьКонецСообщения(ВходныеДанные) Экспорт
	...
	Поток.Перейти(-РазмерСигнатурыКонца, ПозицияВПотоке.Конец);//len-1
	Буфер = Новый БуферДвоичныхДанных(РазмерСигнатурыКонца);
	Поток.Прочитать(Буфер, 0, РазмерСигнатурыКонца);//len
	ПорцияДанных = ПолучитьДвоичныеДанныеИзБуфераДвоичныхДанных(Буфер);
	Если ПорцияДанных = СигнатураПоискаКонца Тогда
		Результат = Истина;
	ИначеЕсли ЭтоПинг(ПорцияДанных) Тогда
		ЭтоПинг = Истина;
		Пока ЭтоПинг Цикл
			Поток.Перейти(-РазмерСигнатурыКонца*2, ПозицияВПотоке.Текущая);//len-2
			Поток.Прочитать(Буфер, 0, РазмерСигнатурыКонца);//len-1
			ПорцияДанных = ПолучитьДвоичныеДанныеИзБуфераДвоичныхДанных(Буфер);
			ЭтоПинг = ЭтоПинг(ПорцияДанных);
		КонецЦикла;
		Результат =  ПорцияДанных = СигнатураПоискаКонца;
	Иначе
		Результат = Ложь;
	КонецЕсли;
	Поток.Закрыть();
